...
exports.createPdfBuffer = void 0;
 ...
 * }
 */
const createPdfBuffer = (data) => new Promise((resolve) => {
    // ---- Helpers -----------------------------------------------------------
    const fmtMoney = (cents, fallback) => {
        if (typeof fallback === "string" && fallback !== "")
            return `$${fallback}`;
        const n = typeof cents === "number" ? cents : 0;
        return `$${(n / 100).toFixed(2)}`;
    };
    const fmtNum = (n, digits = 2) => typeof n === "number" ? n.toFixed(digits) : (0).toFixed(digits);
    const toISO = (d) => {
        if (!d)
            return "";
        const x = new Date(d);
    ...
    const COLS = [
        ...
        { key: "service", label: "Service", width: 150, align: "left" },
        { key: "participant", label: "Participant", width: 140, align: "left" },
        { key: "hours", label: "Hours", width: 55, align: "right" },
        { key: "km", label: "KM", width: 45, align: "right" },
    ...
    // ---- Totals summary ----------------------------------------------------
    const drawTotals = () => {
        const t = safe(data.totals, {});
        const hours = fmtNum(t.hours, 2);
        const km = safe(t.km, 0);
    ...
    const drawTableHeader = () => {
        ...
        // Column titles
        COLS.forEach((c) => {
            const opts = c.align === "right"
                ? { width: c.width, align: "right" }
                : { width: c.width, align: "left" };
            doc
                .font("Helvetica-Bold")
                .fontSize(BODY_FONT)
                .fillColor("#000000")
    ...
    const drawRow = (rowIndex, i) => {
        ...
        COLS.forEach((c) => {
            ...
            const opts = c.align === "right"
                ? { width: c.width - 8, align: "right" }
                : { width: c.width - 8, align: "left" };
            doc
                .font("Helvetica")
                .fontSize(BODY_FONT)
                .fillColor("#111111")
